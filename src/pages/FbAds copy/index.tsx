...
export const groupItemsBy7Days = (data: InputData[]): GroupedOutput[] => {
  ...
  return newData.map((entry) => {
    ...
    const grouped: GroupedItem[] = sortedItems.map((item) => {
...
const FbAds: React.FC = () => {
  ...
  console.log(storeReportFBADSMasterAll)
  ...
  useEffect(() => {
    ...
    if (Array.isArray(storeReportFBADSMaster.data) && storeReportFBADSMaster.data.length !== 0 ) {
      ...
      generateColumns(
        groupItemsBy7Days(storeReportFBADSMaster.data)[1]?.grouped,
 ...
 console.log(savedDataC)
  ...
  const Ex = (data: any) => {
    if (data.criteria_code === "doanh_thu_du_kien") {
      ...
      return Math.floor((parseIntFromEuropeanFormat(groupPrice(groupedReportFBADSMaster, "dau_tu")) / parseIntFromEuropeanFormat(groupPrice(groupedReportFBADSMaster, "tong_tin_nhan")))).toLocaleString("vi-VN")
    }
  ...
  const columns1 = [
    {
      ...
      render: (record: any, data: any) => (
        <Typography content={record + ` (${data.target_unit})`} modifiers={['13x18', '500', 'left']} styles={{ textAlign: "left", fontWeight: 600 , marginLeft:10}} />
      ),
    },
    {
    ...
    },
    {
      ...
      render: (record: any, data: any) => (
        <div >
          ...
          <Typography content={data.target_unit === "VND" ? Ex(data) : data.target_unit === "%" ? Ex(data) + "%" : Ex(data)} modifiers={['13x18', '600', 'left','green']} styles={{ textAlign: "right" }} />
        </div>
      ),
    },
    ...columnsDate
    ...
    const columns2 = [
    {
      title: <Typography content="Chỉ tiêu đánh giá" modifiers={['12x18', '500', 'center', 'blueNavy']} styles={{ textAlign: "left" }} />,
      dataIndex: 'criteria_name',
      align: 'center',
      width: 300, fixed: "left",
      render: (record: any, data: any) => (
  ...
  const TableMemory = useMemo(() => {
    return (
      <PublicTableBusiness
        column={columns1}
        isHideRowSelect
        listData={groupedReportFBADSMaster}
        rowkey="id"
        isPagination={false}
        isExpandable
        showExpandColumn
  ...
  }, [storeisLoadingReportFBADSMaster, groupedReportFBADSMaster, expandedKeys, example, dataReportDate, stateIdArray]);
            ...
            tabLeft={
              <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                  ...
                  {/* <div>
                {
                  ...
                  <div style={{ width: "400px", paddingLeft: "8px", paddingRight: "8px" }}>
